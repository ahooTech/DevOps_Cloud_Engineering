...
if [ "$FIREWALL_TOOL" = "ufw" ]; then
  ...
  if systemctl list-unit-files | grep -q firewalld; then
    ...
    firewall-cmd --permanent --add-rich-rule='rule family="ipv4" source address="10.10.0.0/16" accept' 2>/dev/null || true
...
if [ "$FIREWALL_TOOL" = "ufw" ]; then
  ufw allow proto icmp from 10.10.0.0/16 to any comment "Lab ICMP echo" 2>/dev/null || \
  ufw allow proto icmp from 10.10.0.0/16 to any 2>/dev/null || true
else
  if systemctl list-unit-files | grep -q firewalld; then
    firewall-cmd --permanent --add-rich-rule='rule family="ipv4" source address="10.10.0.0/16" protocol value="icmp" icmp-type value="echo-request" accept' 2>/dev/null || true
    firewall-cmd --reload 2>/dev/null || true
  fi
fi
