...
# Your idea to use VMware Workstation Pro (Free) + Nested ESXi 8.0 (60-day eval) is the only way to get a real vSphere API for free today.
# However, here is the Senior Engineer reality check:
# Hardware Heavy: Running a nested ESXi host requires your laptop to allocate at least 16GB-24GB of RAM just for the VM, plus CPU passthrough (nested virtualization) enabled in BIOS.
# Time Sink: Installing ESXi, configuring the virtual switches, and getting the Terraform provider to authenticate against it can easily take 10-15 hours of troubleshooting network routing issues.
# The 60-Day Clock: The evaluation license expires. If you use this for a portfolio project, you have to rebuild it every two months.

# virtualization/hybrid-vsphere-oci/vsphere/vcenter-config.tf

# Note: The root main.tf already defines the vsphere provider, so we don't 
# need the terraform { required_providers } block here anymore.

# ❌ COMMENT THIS OUT: It requires a live vCenter connection and will crash plan.
/*
...
# Simulated vCenter config for hybrid bridge
locals {
