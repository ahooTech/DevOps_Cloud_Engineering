...
### 1. ✅ KVM and Hyper-V, via Vagrant VMs
...
┌─────────────────────────────────────────────────────────────────────┐
│                     LEGACY & PRIVATE CLOUDS                         │
│  (These sync their state/metadata to the AWS Bridge Node)           │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  ┌─────────────────┐              ┌───────────────────────────────┐ │
│  │  ON-PREM DC     │              │ HCS (Huawei Cloud Stack)      │ │
│  │  (Legacy)       │              │ (Telco Private/Sovereign)     │ │
│  │  • vSphere      │ ────┐        │                               │ │
│  │  • KVM          │     │        │ • Compute Nodes               │ │
│  │  • Hyper-V      │     │        │ • Telco Workloads             │ │
│  └─────────────────┘     │        │ • Safaricom Ecosystem Apps    │ │
│                          │        └───────────────┬───────────────┘ │
│                          │                        │                 │
│                          │  Bridge Sync           │ API/VPN Sync    │
│                          │  (Port 8443)           │ (Port 8443)     │
└──────────────────────────┼────────────────────────┼─────────────────┘
                           │                        │
                           ▼                        ▼
                         ****** infra/hybrid-network*******  
                                      ...
                                      ▼ 
                 ┌──────────────────────────────────────┐
                 │       AWS BRIDGE NODE (Central Hub)  │
                 │       (10.20.1.50 in AWS VPC)        │
                 │                                      │
                 │  • Aggregates sync from On-Prem & HCS│
                 │  • Routes to OCI Bare-Metal          │
                 │  • Enforces CIS Hardening & Parity   │
                 └──────────────┬───────────────────────┘
                                │
        ┌───────────────────────┼───────────────────────
        │                       │                       │
